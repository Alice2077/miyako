...
async def add_dlc(bot, ev: CQEvent):
    ...
    if not priv.check_priv(ev, priv.OWNER):
        await bot.finish(ev, '只有群主才能加载dlc哦。', at_sender=False)
    args = ev.message.extract_plain_text().split()
    if len(args)>= 2:
        await bot.finish(ev, '指令格式错误。', at_sender=False)
    if len(args)==0:
        await bot.finish(ev, '请输入加载dlc+dlc名。', at_sender=False)
    dlcname = args[0]
    if dlcname not in dlcdict.keys():
        await bot.finish(ev, 'DLC名填写错误。', at_sender=False)        

    if gid in dlc_switch[dlcname]:
        await bot.finish(ev, '本群已开启此dlc哦。', at_sender=False)
    dlc_switch[dlcname].append(gid)
    save_dlc_switch()
    await bot.finish(ev, f'加载dlc {dlcname}  成功!', at_sender=False)
...
async def delete_dlc(bot, ev: CQEvent):
    gid = ev.group_id
    if not priv.check_priv(ev, priv.OWNER):
        await bot.finish(ev, '只有群主才能卸载dlc哦。', at_sender=False)
    args = ev.message.extract_plain_text().split()
    if len(args)>= 2:
        await bot.finish(ev, '指令格式错误', at_sender=False)
    if len(args)==0:
        await bot.finish(ev, '请输入卸载dlc+dlc名。', at_sender=False)
    dlcname = args[0]
    if dlcname not in dlcdict.keys():
        await bot.finish(ev, 'DLC名填写错误', at_sender=False)        

    if gid not in dlc_switch[dlcname]:
        await bot.finish(ev, '本群没有开启此dlc哦。', at_sender=False)
    dlc_switch[dlcname].remove(gid)
    save_dlc_switch()
    await bot.finish(ev, f'卸载dlc {dlcname}  成功!', at_sender=False)
...
async def noblelogin(bot, ev: CQEvent):
    ...
    if not daily_sign_limiter.check(guid):
        await bot.send(ev, '今天已经签到过了哦，明天再来吧。', at_sender=False)
        return
    duel = DuelCounter()
...
async def add_girl(bot, ev: CQEvent):
    ...
    if duel._get_level(gid, uid) == 0:
        ...
        if len(newgirllist) == 0:
            if cidnum!=0:
                await bot.send(ev, '这个群已经没有可以约到的新女友了哦。', at_sender=False)
                return        
            else : 
...
async def nobleduel(bot, ev: CQEvent):
    ...
    is_overtime = 0
    if id2 == id1:
        await bot.send(ev, "不能和自己决斗！", at_sender=False)
        duel_judger.turn_off(ev.group_id)
        return 
    ...
    if not daily_duel_limiter.check(guid):
        await bot.send(ev, '今天的决斗次数已经超过上限了哦，明天再来吧。', at_sender=False)
        duel_judger.turn_off(ev.group_id)
        return
...
async def cheat_score(bot, ev: CQEvent):
    if not priv.check_priv(ev, priv.SUPERUSER):
        await bot.finish(ev, '不要想着走捷径哦。', at_sender=False)
    gid = ev.group_id
    match = ev['match']
...
async def init_duel(bot, ev: CQEvent):
    if not priv.check_priv(ev, priv.ADMIN):
        await bot.finish(ev, '只有群管理才能使用重置决斗哦。', at_sender=False)
    duel_judger.turn_off(ev.group_id)
    msg = '已重置本群决斗状态！'
    await bot.send(ev, msg, at_sender=False)
...
async def search_girl(bot, ev: CQEvent):
    ...
    gid = ev.group_id
    if not args:
        await bot.send(ev, '请输入查女友+pcr角色名。', at_sender=False)
        return
    name = args[0]
    cid = chara.name2id(name)
    if cid == 1000:
        await bot.send(ev, '请输入正确的pcr角色名。', at_sender=False)
        return
    duel = DuelCounter()
    ...
    if duel._get_queen_owner(gid,cid) !=0 :
        ...
        await bot.finish(ev, f'\n{c.name}现在是\n[CQ:at,qq={owner}]的皇后哦。{mes}', at_sender=False)

    if owner == 0:
        await bot.send(ev, f'{c.name}现在还是单身哦，快去约到她吧。{mes}', at_sender=False)
        return
    else:
...
async def reset_score(bot, ev: CQEvent):
    gid = ev.group_id
    if not priv.check_priv(ev, priv.OWNER):
        await bot.finish(ev, '只有群主才能使用重置金币功能哦。', at_sender=False)
    args = ev.message.extract_plain_text().split()
    if len(args)>=2:
        await bot.finish(ev, '指令格式错误', at_sender=False)
    if len(args)==0:
        await bot.finish(ev, '请输入重置金币+被重置者QQ', at_sender=False)
    else :
        id = args[0]
        duel = DuelCounter()
        if duel._get_level(gid, id) == 0:
...
async def reset_chara(bot, ev: CQEvent):
    gid = ev.group_id
    if not priv.check_priv(ev, priv.OWNER):
        await bot.finish(ev, '只有群主才能使用重置角色功能哦。', at_sender=False)
    args = ev.message.extract_plain_text().split()
    if len(args)>=2:
        await bot.finish(ev, '指令格式错误', at_sender=False)
    if len(args)==0:
        await bot.finish(ev, '请输入重置角色+被重置者QQ', at_sender=False)
    else :
        id = args[0]
        duel = DuelCounter()
        if duel._get_level(gid, id) == 0:
...
async def breakup(bot, ev: CQEvent):
    if BREAK_UP_SWITCH == True:
        ...
        if level == 0:
            await bot.finish(ev, '该用户还未在本群创建贵族哦。', at_sender=True)
        if not args:
            await bot.finish(ev, '请输入分手+pcr角色名。', at_sender=False)
        name = args[0]
        cid = chara.name2id(name)
        if cid == 1000:
            await bot.finish(ev, '请输入正确的pcr角色名。', at_sender=False)
        score_counter = ScoreCounter2()
        needscore = 400+level*100
        score = score_counter._get_score(gid, uid)
        cidlist = duel._get_cards(gid, uid)
        if cid not in cidlist:
            await bot.finish(ev, '该角色不在你的身边哦。', at_sender=False)
        #检测是否是皇后
        queen = duel._search_queen(gid,uid)
        if cid==queen:
            await bot.finish(ev, '不可以和您的皇后分手哦。', at_sender=False)
        if score < needscore:
...
async def open_prestige(bot, ev: CQEvent):
    ...
    prestige = score_counter._get_prestige(gid,uid)
    if prestige != None:
        await bot.finish(ev, '您已经开启了声望系统哦。', at_sender=False)    
    if level != 6:
        await bot.finish(ev, '只有国王才可以开启声望系统哦。', at_sender=False)    
...
async def inquire_prestige(bot, ev: CQEvent):
    ...
    prestige = score_counter._get_prestige(gid,uid)
    if prestige == None:
        await bot.finish(ev, '您还未开启声望系统哦。', at_sender=False)
...
async def be_emperor(bot, ev: CQEvent): 
    ...
    if prestige == None:
        await bot.finish(ev, '您还未开启声望系统哦。', at_sender=False)
    if level!=6:
        await bot.finish(ev, '只有国王才能进行加冕仪式哦。', at_sender=False)
  
    if prestige < 5000: 
        await bot.finish(ev, '加冕仪式需要5000声望，您的声望不足哦。', at_sender=False)
    score = score_counter._get_score(gid, uid)
    if score < 5000:
        await bot.finish(ev, '加冕仪式需要5000金币，您的金币不足哦。', at_sender=False)
...
async def marry_queen(bot, ev: CQEvent):
    ...
    if prestige == None:
        await bot.finish(ev, '您还未开启声望系统哦。', at_sender=False)    
    if level != 7:
        await bot.finish(ev, '只有皇帝才可以举办皇室婚礼哦。', at_sender=False)  
    if duel._search_queen(gid,uid)!=0:
        await bot.finish(ev, '皇帝只可以举办一次皇室婚礼哦。', at_sender=False)
    if not args:
        await bot.finish(ev, '请输入皇室婚礼+pcr角色名。', at_sender=False)
    name = args[0]
    cid = chara.name2id(name)
    if cid == 1000:
        await bot.finish(ev, '请输入正确的pcr角色名。', at_sender=False)
    ...
    if cid not in cidlist:
        await bot.finish(ev, '该角色不在你的身边哦。', at_sender=False)        
    if prestige < 3000: 
        await bot.finish(ev, '皇室婚礼需要3000声望，您的声望不足哦。', at_sender=False)
    score = score_counter._get_score(gid, uid)
    if score < 3000:
        await bot.finish(ev, '皇室婚礼需要3000金币，您的金币不足哦。', at_sender=False)
    favor = duel._get_favor(gid,uid,cid)
    if favor < 300:
        await bot.finish(ev, '举办婚礼的女友需要达到300好感，您的好感不足哦。', at_sender=False)        
...
async def girl_story(bot, ev: CQEvent):
    ...
    duel = DuelCounter()
    if not args:
        await bot.finish(ev, '请输入查好感+女友名。', at_sender=False)
    name = args[0]
    cid = chara.name2id(name)
    if cid == 1000:
        await bot.finish(ev, '请输入正确的女友名。', at_sender=False)
    cidlist = duel._get_cards(gid, uid) 
    if cid not in cidlist:
        await bot.finish(ev, '该女友不在你的身边哦。', at_sender=False)
...
async def daily_date(bot, ev: CQEvent):
    ...
    duel = DuelCounter()
    if not args:
        await bot.finish(ev, '请输入贵族约会+女友名。', at_sender=False)
    name = args[0]
    cid = chara.name2id(name)
    if cid == 1000:
        await bot.finish(ev, '请输入正确的女友名。', at_sender=False)
    ...
    if not daily_date_limiter.check(guid):
        await bot.finish(ev, '今天已经和女友约会过了哦，明天再来吧。', at_sender=False)
...
async def give_gift(bot, ev: CQEvent):
    ...
    if len(args)!=2:
        await bot.finish(ev, '请输入 送礼物+女友名+礼物名 中间用空格隔开。', at_sender=False)
    name = args[0]
    cid = chara.name2id(name)
    if cid == 1000:
        await bot.finish(ev, '请输入正确的女友名。', at_sender=False)
    ...
    if gift not in GIFT_DICT.keys():
        await bot.finish(ev, '请输入正确的礼物名。', at_sender=False)
    gfid = GIFT_DICT[gift]
    if duel._get_gift_num(gid,uid,gfid)==0:
        await bot.finish(ev, '你的这件礼物的库存不足哦。', at_sender=False)
...
async def buy_gift(bot, ev: CQEvent):
    ...
    score = score_counter._get_score(gid, uid)
    if score < 300:
        await bot.finish(ev, '购买礼物需要300金币，您的金币不足哦。', at_sender=False) 
    if not daily_gift_limiter.check(guid):
        await bot.finish(ev, f'今天购买礼物已经超过{GIFT_DAILY_LIMIT}次了哦，明天再来吧。', at_sender=False)     
    select_gift = random.choice(list(GIFT_DICT.keys()))
    gfid = GIFT_DICT[select_gift]
    duel._add_gift(gid,uid,gfid)
    msg = f'\n您花费了300金币，\n买到了[{select_gift}]哦，\n欢迎下次惠顾。'
...
async def my_gift(bot, ev: CQEvent):
    ...
    for gift in GIFT_DICT.keys():
        ...
        #补空格方便对齐
            ...
            giftmsg+=f'\n{gift}{msg_part}: {num}件'
    if giftmsg == '':
        await bot.finish(ev, '您现在没有礼物哦，快去发送 买礼物 购买吧。', at_sender=False)        
    msg+=giftmsg
...
async def change_gift(bot, ev: CQEvent):
    ...
    if id2 == id1:
        await bot.send(ev, "不能和自己交换礼物！", at_sender=False)
        gift_change.turn_off_giftchange(ev.group_id)
        return             
    gift1 = match.group(1)
    gift2 = match.group(3)
    ...
    if gfid2 == gfid1:
        await bot.send(ev, "不能交换相同的礼物！", at_sender=False)
        gift_change.turn_off_giftchange(ev.group_id)
        return           
...
async def buy_information(bot, ev: CQEvent):
    ...
    if duel_judger.get_on_off_status(ev.group_id):
        ...
        await bot.finish(ev, msg, at_sender=False)    
    if not args:
        await bot.finish(ev, '请输入买情报+女友名。', at_sender=False)
    name = args[0]
    cid = chara.name2id(name)
    if cid == 1000:
        await bot.finish(ev, '请输入正确的女友名。', at_sender=False)
    score = score_counter._get_score(gid, uid)
    if score < 500:
        await bot.finish(ev, '购买女友情报需要500金币，您的金币不足哦。', at_sender=False)    
...
async def init_change(bot, ev: CQEvent):
    if not priv.check_priv(ev, priv.ADMIN):
        await bot.finish(ev, '只有群管理才能使用重置礼物交换哦。', at_sender=False)
    gift_change.turn_off_giftchange(ev.group_id)
    msg = '已重置本群礼物交换状态！'
    await bot.send(ev, msg, at_sender=False)
...
async def get_favorlist(bot, ev: CQEvent):
    ...
    cidlist = duel._get_cards(gid, uid)
    if len(cidlist)==0:
        await bot.finish(ev, '您现在还没有女友哦。', at_sender=False)
    favorlist = []
    for cid in cidlist:
        favor = duel._get_favor(gid,uid,cid)
    ...
    if len(favorlist)==0:
        await bot.finish(ev, '您的女友好感全部为0哦。', at_sender=False)        
...
async def add_warehouse(bot, ev: CQEvent):
    ...
    if duel_judger.get_on_off_status(ev.group_id):
        ...
        await bot.finish(ev, msg, at_sender=False)    
    if level < 6:
        await bot.finish(ev, '只有国王以上才能增加女友上限哦。', at_sender=False)
...
async def get_girlname(bot, ev: CQEvent):
    ...
    duel = DuelCounter()
    if not args:
        await bot.finish(ev, '请输入查名字+女友序号(序号即查询贵族中女友排在第几位)。', at_sender=False)
    num = args[0]
    if not num.isdigit():
        await bot.finish(ev, '请输入查名字+女友序号(序号即查询贵族中女友排在第几位)。', at_sender=False)
    num_int = int(num)
    ...
    if num_int>girlnum:
        await bot.finish(ev, f'你所输入的女友序号{num_int}大于您的女友数{girlnum}名', at_sender=False)
